...
import_other_yem_ocb <- function(path_linelist_other, dict_linelist) {
  ...
  # prep mapping
  df_map <- file.path(path_to_files, "LL_v2.1_mapping_template_OCB_YEM_v2.xlsx") %>% 
    readxl::read_xlsx() %>% 
    janitor::clean_names() %>% 
    select(1, 6, 7, 8, 9, 10) %>% 
  ...
  # prep linelist files (IPD, ICU, MOH)
  file_ipd <- llutils::list_files(
    path_to_files,
    pattern = "YE155_Register_IPD",
    select = "latest"
  )
  
  file_icu <- llutils::list_files(
    path_to_files,
    pattern = "YE155_Register_ICU",
    select = "latest"
  )
  
  file_moh <- llutils::list_files(
    path_to_files,
    pattern = "YE155_Line List_MoH",
    select = "latest"
  )
  
  
  d_ipd <- suppressMessages(readxl::read_xlsx(file_ipd, col_types = "text", skip = 1)) %>% 
    select(-starts_with("...")) %>% 
    setNames(hmatch::string_std(names(.))) %>% 
    janitor::remove_empty("rows") %>% 
    mutate(site = "YEM_B_GAM") %>% 
    dplyr::left_join(dict_facilities_join, by = "site") %>% 
    mutate(upload_date = as.character(llutils::extract_date(file_ipd)))
