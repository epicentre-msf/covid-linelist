...
import_linelists <- function(path_data_raw,
  ...
  df_sheets <- scan_sheets(path_data_raw, country = country)
  ...
  ## import and prepare
  df_data <- df_sheets %>%
    group_by(country, OC, project, upload_date) %>% 
    do(read_and_prepare_data(file_path = .$file_path)) %>%
    ungroup() %>% 
    # TODO: improve this derivation of site via left_join below
    left_join(dict_facilities, by = c("country", "OC", "project", "MSF_name_facility")) %>% 
    mutate(patient_id = paste(site, MSF_N_Patient, sep = "-")) %>% 
    mutate(db_row = 1:n())
...
scan_sheets <- function(path_data_raw, country) {
  ...
  df_sheet <- tibble::tibble(file_path = files_country) %>%
    mutate(path_parse = gsub("^linelist_Covid_anonymous__|_[[:digit:]]{2}-[[:digit:]]{2}\\.xlsb", "", file_path)) %>% 
...
read_and_prepare_data <- function(file_path) {
  ...
  if (any(stringr::str_detect(names(df), "^\\.\\."))) {
    warning("Missing column names in file: ", file_path, immediate. = TRUE)
  }
