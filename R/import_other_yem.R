...
import_other_yem <- function(path_linelist_other) {
  ...
  d_orig <- dplyr::bind_rows(
    import_yem_helper(file_sana, "YEM_P_SAN", project = "Y131", site_name = "Al Kuwait Hospital"),
    import_yem_helper(file_aden, "YEM_P_ADE", project = "Ye133", site_name = "Al Amal Hospital")
  )
  ...
  d_recode <- d_orig %>% 
    rename(MSF_N_Patient = number_patient,
           ...
           # x32,
           MSF_received_oxygen = received_oxygen,
           # received_non_invasive_ventilation_cpap,
           patcourse_vent = received_mechanical_ventilation,
           # x36,
  ...
  d_clean <- d_recode %>% 
    ...
    mutate(report_country = "YEM",
           ...
           patcourse_icu = ifelse(!is.na(date_admitted_to_icu_dd_mm_yyyy) | !is.na(date_discharged_from_icu_dd_mm_yyyy), "Yes", "No"),
           outcome_patcourse_icu = patcourse_icu,
           outcome_patcourse_vent = patcourse_vent,
           MSF_outcome_received_oxygen = MSF_received_oxygen,
...
import_yem_helper <- function(path, site, project, site_name) {
  
  upload_date <- as.character(
    as.Date(
  ...
  readxl::read_xlsx(path, sheet = "LL-COVID19", skip = 6, col_types = "text") %>% 
    janitor::clean_names() %>% 
